...
const double NewFourLevelPlugin::hbar = 1.05457148e-34;
const double NewFourLevelPlugin::pi = 3.1415926535897931;
const double NewFourLevelPlugin::eps0 = 8.854187817e-12;
const double NewFourLevelPlugin::c = 2.99792458e8;

const char* NewFourLevelPlugin::names[16] = {   "w a",          //0
                                                "gamma a",      //1
                                                "w b",          //2
                                                "gamma b",      //3
                                                "t30",          //4
...
void NewFourLevelPlugin::initialize(const double** parameters, double dt)
{
    //loop over 3 axes
    for(int i=0; i<3; i++){
        ...
        // calculate rate equation update coefficients         
        cn1[i] = (2.-wa*wa*dt*dt)/(1.+0.5*ga*dt);
        cn1m1[i] = -(1.-0.5*ga*dt)/(1.+0.5*ga*dt);
        cn1e[i] = (dt*dt*Ndensity/eps0)/(1.+0.5*ga*dt) *
                 6.*pi*eps0*c*c*c/(wa*wa*t21[i]);
                          
        cn2[i] = (2.-wa*wa*dt*dt)/(1.+0.5*ga*dt);
        cn2m1[i] = -(1.-0.5*ga*dt)/(1.+0.5*ga*dt);
        cn2e[i] = (dt*dt*Ndensity/eps0)/(1.+0.5*ga*dt) *
                 6.*pi*eps0*c*c*c/(wa*wa*t21[i]);
                          
        cn3[i] = (2.-wa*wa*dt*dt)/(1.+0.5*ga*dt);
        cn3m1[i] = -(1.-0.5*ga*dt)/(1.+0.5*ga*dt);
        cn3e[i] = (dt*dt*Ndensity/eps0)/(1.+0.5*ga*dt) *
                 6.*pi*eps0*c*c*c/(wa*wa*t21[i]);
...
void NewFourLevelPlugin::initializeStorageE(int axis, float* storage)
{
...
float NewFourLevelPlugin::calculate(int i, float U, float V, float Ef,float *storage)
{
    double E = double(Ef);
    ...
    double N0n = storage[4];
    double N1n = storage[5];
    double N2n = storage[6];
    double N3n = storage[7];

    double N0nm1 = storage[8];
    double N1nm1 = storage[9];
    double N2nm1 = storage[10];
    double N3nm1 = storage[11];

    //Step 1
    double Panp1 = ca1[i]*Pan + ca2[i]*Panm1 + ca4[i]*(N1n-N2n)*E; //N2, N1 are offset in time by 1/2 time step, ignored here
    double Pbnp1 = cb1[i]*Pbn + cb2[i]*Pbnm1 + cb4[i]*(N0n-N3n)*E; //N3, N0 are offset in time by 1/2 time step, ignored here
    ...
    //Step 3
    // My method
    double N3np1 = () + N3nm1 + E*(Pbnp1-Pbnm1)*oneOverNormalizedWb[i]
    ...
    storage[6] = float(N2n);
    storage[7] = float(N3n);
    storage[8] = float(N0nm1);
    storage[9] = float(N1nm1);
    storage[10] = float(N2nm1);
    storage[11] = float(N3nm1);

    return float(Enp1);
}

float NewFourLevelPlugin::calculateEx( float U, float V, float Ex, float* storage )
{
    return calculate(0, U, V, Ex, storage);
}

float NewFourLevelPlugin::calculateEy( float U, float V, float Ey, float* storage )
{
    return calculate(1, U, V, Ey, storage);
}

float NewFourLevelPlugin::calculateEz( float U, float V, float Ez, float* storage )
{
    return calculate(2, U, V, Ez, storage);
}

MATERIAL_PLUGIN(NewFourLevelPlugin);
