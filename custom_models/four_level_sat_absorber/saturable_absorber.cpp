...
const double SaturableAbsorberPlugin::hbar = 1.05457148e-34;
const double SaturableAbsorberPlugin::pi = 3.1415926535897931;
const double SaturableAbsorberPlugin::eps0 = 8.854187817e-12;
const double SaturableAbsorberPlugin::c = 2.99792458e8;

const char* SaturableAbsorberPlugin::names[16] = {"w a",          //0
                                                     "gamma a",      //1
                                                     "w b",          //2
                                                     "gamma b",      //3
                                                     "t30",          //4
...
void SaturableAbsorberPlugin::initialize(const double** parameters, double dt)
{
    //loop over 3 axes
    for(int i=0; i<3; i++){
...
void SaturableAbsorberPlugin::initializeStorageE(int axis, float* storage)
{
...
float SaturableAbsorberPlugin::calculate(int i, float U, float V, float Ef,float *storage)
{
    double E = double(Ef);
...
float SaturableAbsorberPlugin::calculateEx( float U, float V, float Ex, float* storage )
{
    return calculate(0, U, V, Ex, storage);
}

float SaturableAbsorberPlugin::calculateEy( float U, float V, float Ey, float* storage )
{
    return calculate(1, U, V, Ey, storage);
}

float SaturableAbsorberPlugin::calculateEz( float U, float V, float Ez, float* storage )
{
    return calculate(2, U, V, Ez, storage);
}

MATERIAL_PLUGIN(SaturableAbsorberPlugin);
